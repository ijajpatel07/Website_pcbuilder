...
if(isset($_POST["userid"])){
    ...
    $emailTo = $_POST["userid"];
    $code = uniqid(true);
    $query = mysqli_query($con,"INSERT INTO resetpasswords(code, userid) VALUES('$code','$emailTo')");
    if(!$query) {
        exit("Error");
    }
    ...
    try {
        ...
        $mail->Username   = "pcbuilder528@gmail.com";                     // SMTP username
        $mail->Password   = "pcbuilder242837";                               // SMTP password
        ...
        //Recipients
        $mail->setFrom('atharva.mitragotri@gmail.com', 'PC Builder');
        $mail->addAddress($emailTo);     // Add a recipient
        $mail->addReplyTo('noreply@pcbuilder.com', 'No reply');
        ...
        if(alert("Reset password link has been sent to your email")){
        window.location="login.php";}
        </script>
        <?php
    } catch (Exception $e) {
        echo "<script>alert('Message could not be sent. Mailer Error: {$mail->ErrorInfo}');//window.location='resetpassword.php';</script>";
    }
    exit();
}
?>
